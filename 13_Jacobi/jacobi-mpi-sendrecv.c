	...
	for (k=0;k<MAX_ITERATIONS;k++) {
		...
		if (myrank > 0) {
			...
			//MPI_Recv(&grid[0][1], ny, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
		        MPI_Sendrecv(&grid[1][1], ny, MPI_DOUBLE, myrank-1, 0, &grid[0][1], ny, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
		}
		if (myrank < size-1) {
			...
			//MPI_Send(&grid[local_nx][1], ny, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD);
		        MPI_Sendrecv(&grid[local_nx][1], ny, MPI_DOUBLE, myrank+1, 0, &grid[local_nx+1][1], ny, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
