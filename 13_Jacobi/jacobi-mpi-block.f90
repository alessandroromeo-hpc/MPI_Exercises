...
program jacobi
  ...
  do k=0, MAX_ITERATIONS
     ...
     ! left halo
     if (myrank .gt. 0) then
        call mpi_send(grid(1,1), ny, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD,  ierr)
        call mpi_recv(grid(1,0), ny, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD, status, ierr)
     end if
     ...
     if (myrank .lt. size-1) then
        call mpi_send(grid(1,local_nx), ny, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD,  ierr)
        call mpi_recv(grid(1,local_nx+1), ny, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD,status,  ierr)
     end if
     tmpnorm=0.0
     do j=1, local_nx
        do i=1, ny
