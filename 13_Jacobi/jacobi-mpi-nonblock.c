...
int main(int argc, char * argv[]) {
    ...
    // initialise global grids and local grid
    ...
    // Collect the norm factors from all processes and sum them up
	...
	// Halo exchange using non-blocking send/recv
        if (myrank > 0) {
            MPI_Isend(&grid[1][1], ny, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD, &requests[0]);
            MPI_Irecv(&grid[0][1], ny, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD, &requests[1]);
        }
        if (myrank < size-1) {
            MPI_Isend(&grid[local_nx][1], ny, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD, &requests[2]);
            MPI_Irecv(&grid[local_nx+1][1], ny, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD, &requests[3]);
        }
        MPI_Waitall(4, requests, MPI_STATUSES_IGNORE);

	// Convergence check - sum norm values from each local grid.
        tmpnorm=0.0;
        for (i=1; i<=local_nx; i++) {
            for (j=1; j<ny+1; j++) {
                tmpnorm=tmpnorm+pow(grid[i][j]*4-grid[i][j-1]-grid[i][j+1]-grid[i-1][j]-grid[i+1][j],2);
	...
	// update new grid with 4-point stencil
        for (i=1; i<=local_nx; i++) {
            for (j=1; j<ny+1; j++) {
