...
int main(int argc, char* argv[]){
  ...
  MPI_Sendrecv(&matrix[N], N, MPI_INTEGER, proc_up, 10, &matrix[N*((num_local_row+1))], N, MPI_INTEGER, proc_down, 10, MPI_COMM_WORLD, &status1);
  MPI_Sendrecv(&matrix[N*num_local_row], N, MPI_INTEGER, proc_down, 11, &matrix[0], N, MPI_INTEGER, proc_up, 11, MPI_COMM_WORLD, &status2);
